...
def basic_series() -> pd.Series:
    ...
    """
    nums = [1,2,3,4,5]
    return pd.Series(nums, dtype=int, name='Fred')
...
def float_series() -> pd.Series:
    ...
    """
    return pd.Series(list(arange(0.000, 1.001, 0.001)), dtype=float)
...
def alpha_index_series() -> pd.Series:
    ...
    """
    return pd.Series(list(range(1,27)), index=list(string.ascii_lowercase), dtype=np.int64)
...
def object_values_series() -> pd.Series:
    ...
    Don't worry about the series name.
    """
    return pd.Series(data=list(string.ascii_uppercase), index=list(range(101, 127)), dtype=object)
