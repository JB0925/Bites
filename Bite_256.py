...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    pycons = []
    soup = Soup(data, 'lxml')
    info = soup.findAll(text=True)
    
    for item in info:
        try:
            if item in '0123456789' or item in ['2018', '2019', '2020']:
                pass
            else:
                item = json.loads(item)
                if 'pycon' in item['name'].lower():
                    name = item['name']
                    city = item['location']['address']['addressLocality']
                    country = item['location']['address']['addressCountry']
                    start_date = parse(item['startDate'])
                    #print(f'Name: {name}, City: {city}, Start: {start_date}')
                    end_date = parse(item['endDate'])
                    url = item['url']
                    p = PyCon(name=name, city=city, country=country, start_date=start_date,\
                        end_date=end_date, url=url)
                    pycons.append(p)
        except:
            pass
    
    return sorted(pycons, key=lambda x: x.start_date)
...
def filter_pycons(pycons: List[PyCon],
    ...
    that continent.
    """
    return [p for p in pycons if p.start_date.year == year and get_continent(p.country)\
            == continent]
